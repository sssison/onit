...
async def tbot_vision_search_and_approach_object(
    ...
    last_detection = {
        "visible": bool(initial_search.get("found", False)),
        "confidence": float(initial_search.get("confidence", 0.0) or 0.0),
        "position": initial_search.get("position"),
        "bbox": initial_search.get("bbox"),
    }
            ...
            try:
                ...
                if front_distance is None:
                    ...
                    return {
                        ...
                        "timing_s": time.monotonic() - started,
                        "errors": [*errors, "LiDAR front distance unavailable"],
                    }

                if front_distance <= effective_target_distance_m:
                    await _safe_motion_stop(motion)
                    return {
                        "status": "reached",
                        "object_name": object_name_clean,
                ...
                approach_result = _extract_tool_result_dict(approach_raw)
                counters["forward_segments"] += 1

                approach_status = str(approach_result.get("status") or "")
                approach_front_distance = approach_result.get("front_distance")
                ...
                if approach_status not in ("reached", "completed"):
                    await _safe_motion_stop(motion)
                    return {
                        "status": "error",
                        "object_name": object_name_clean,
                ...
                return {
                    ...
                    "timing_s": time.monotonic() - started,
                    "errors": errors or None,
                    "approach_move_posts": approach_result.get("move_posts"),
                }
            except Exception as e:
                await _safe_motion_stop(motion)
                return {
